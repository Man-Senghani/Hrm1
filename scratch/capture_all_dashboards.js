...
async function capture() {
  ...
  for (const portal of portals) {
    console.log(`=== Processing ${portal.role.toUpperCase()} ===`);
    ...
    try {
      ...
      console.log(`✅ [LIGHT] Saved: ${portal.lightFile}`);
      await pageLight.close().catch(() => {});
    } catch(err) {
      console.error(`❌ [LIGHT ERROR] ${portal.role}:`, err.message);
    }
    ...
    try {
      ...
      console.log(`✅ [DARK]  Saved: ${portal.darkFile}`);
      await pageDark.close().catch(() => {});
    } catch(err) {
      console.error(`❌ [DARK ERROR] ${portal.role}:`, err.message);
    }
  }

  await browser.close().catch(() => {});
  console.log('🎉 ALL 8 DASHBOARD SCREENSHOTS CAPTURED SUCCESSFULLY!');
}
