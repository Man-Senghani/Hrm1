...
async function capture() {
  ...
  for (const t of targets) {
    console.log(`📸 Capturing ${t.role.toUpperCase()}...`);
    try {
      ...
      await page.goto(t.url, { waitUntil: 'networkidle2', timeout: 20000 }).catch(async () => {
        await page.goto(t.url, { waitUntil: 'domcontentloaded', timeout: 10000 }).catch(() => {});
      });
      ...
      console.error(`❌ Error in ${t.role}:`, err.message);
    }
  }

  await browser.close();
  console.log('🎉 Done capturing all 4 portals!');
  process.exit(0);
}
