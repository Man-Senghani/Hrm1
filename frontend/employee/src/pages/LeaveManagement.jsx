...
import {
  ...
  ArrowRight, User, FileText, ChevronLeft, ChevronRight, MoreHorizontal, CalendarDays, ChevronDown, Edit, Sparkles, RefreshCw
} from 'lucide-react';
...
const LeaveManagement = ({ isChild = false }) => {
  ...
  const approvedLeavesArray = leaves.filter(l => l.status?.toLowerCase() === 'approved' && isUpToRefDate(l));
          ...
          <div className="bg-white dark:bg-[#111c18] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-5 flex flex-col justify-start gap-2 transition-colors duration-300 hover:!border-emerald-500 dark:hover:!border-emerald-400">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left">
                ...
                <tbody>
                  {[
                    { name: 'Casual Leave (CL)', balance: casualBalance, used: usedCasual, total: casualTotal, icon: Calendar, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-100 dark:border-emerald-900/40' },
                    { name: 'Sick Leave (SL)', balance: sickBalance, used: usedSick, total: sickTotal, icon: FileText, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-950/60 border border-blue-100 dark:border-blue-900/40' }
                  ].map((row, idx) => (
